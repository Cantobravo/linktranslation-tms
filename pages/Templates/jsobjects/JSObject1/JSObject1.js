export default {
  ...
  // Append token to the correct body field based on type
  async appendToken(mode, token) {
    const cur = this.get(mode);
    const type = String(cur.template_type || "");
    if (type.includes("email")) {
      await this.set(mode, "template_email_body_html", (cur.template_email_body_html || "") + token);
    } else if (type.includes("pdf")) {
      await this.set(mode, "template_header_html", (cur.template_header_html || "") + token);
    } else {
